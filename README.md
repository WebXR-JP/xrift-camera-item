...
- **4-SPLIT** — モニタを 4 分割して全カメラを同時に見る（下記「マルチビュー」参照）
- **カット切替チップ** — `● カット: 瞬時切替` / `● カット: 飛んで移動` を切り替える（下記「ハードカット」参照）
...
カットの切り替わり方は 2 種類から選べます。モニタ下の「カット切替チップ」で切り替え、選択はインスタンス全体に同期されます。

| 設定 | 挙動 |
|---|---|
| `瞬時切替`（cut） | カットの瞬間、**絵だけ**がパッと入れ替わる。放送カメラのハードカットと同じ。機体は飛んで移動を続けるため、他の参加者から見たドローンの動きは自然のまま |
| `飛んで移動`（既定） | カットのたびに機体が次の撮影位置へ飛んでいく。ワールド内に実体があるアイテムとしての挙動 |

瞬時切替では、カメラだけがショットの理想姿勢へ一瞬で送られます（内部ではリグのスナップを使い、手ブレなどの平滑化もリセットしません）。どのクライアントでも同じカット情報から同じ瞬間に切り替わります。

### 4 分割マルチビュー

モニタ下の **4-SPLIT** ボタンで、モニタが放送局のマルチビューモニタになります。

| チャンネル | 内容 |
|---|---|
| CAM1（左上） | プログラム。メインのドローン視点（HUD 付き） |
| CAM2（右上） | 主役以外の注目度上位者 1 人を寄り（CU）で |
| CAM3（左下） | 別の人を回り込み（ORB）で |
| CAM4（右下） | その場の全員を収める全体画（WS） |

- チャンネルへの割り当ては**参加者 ID とサーバ時計から決定論的に計算**されるため、新しい同期項目は不要。全員の画面で同じ分割になります
- CAM2〜4 は実在する機体を持たない仮想カメラです（ドローンは 1 台のまま）
- 表示は**閲覧者ごと**の切り替えで同期されません。他人の画面まで分割されることはありません
- 各チャンネルは 10fps で更新。録画は CAM1（プログラム）のみで、マルチビューは録られません
...
src/
  ...
  camera/
    ...
    multiview.ts            モニタ用 4 分割マルチビュー（scissor で 4 パス描画）
    hud.ts                  ビューファインダ HUD（2D キャンバス）
    useRecorder.ts          ピクセルリードバック → 合成 → WebCodecs(mp4) / MediaRecorder(webm)
    mp4Recorder.ts          WebCodecs VideoEncoder + mp4-muxer による .mp4 録画
